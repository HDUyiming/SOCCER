...
class FastRCNNLossComputation(object):
    ...
    def subsample(self, proposals, targets, class_logits = None, SOCCER_mode = 'none'):
        """
        ...
        if SOCCER_mode == 'Stu_use_Tea_pred_to_pred_again':
            class_prob = F.softmax(class_logits, -1)
            proposals_per_image.add_field("probs", class_prob)
            only_pos_proposals = [proposals_per_image[class_inds]]
            self._proposals = only_pos_proposals
            return only_pos_proposals   
    ...
    def __call__(self, class_logits, box_regression, use_pseudo_labeling_weight='none', SOCCER_mode = 'none'):
        """
        Computes the loss for Faster R-CNN.
        ...
        if SOCCER_mode == 'Stu_consis':
            n = torch.abs(box_regression[sampled_pos_inds_subset[:, None], map_inds] - regression_targets[sampled_pos_inds_subset])
            n_mean = n.mean().item()

            self.n_add1 = self.n_add1 + n_mean
            n_avg1 = self.n_add1 / self.count1
            self.count1 +=1
            if n_avg1 < 0.6:
                n_avg1 = 0.6
            # n_avggg = 1- n_avg1
            huber_loss = torch.nn.HuberLoss(reduction='none', delta = n_avg1)
            losses = huber_loss(
                box_regression[sampled_pos_inds_subset[:, None], map_inds],
                regression_targets[sampled_pos_inds_subset]
            )
            box_loss = torch.sum(losses) / labels.numel()
